...
class GameState:
    ...
    def __init__(self, screen):
        ...
        fontName = pygame.font.match_font('bookman')
        self.font1 = pygame.font.Font(fontName, 72)
        ...
        self.font2 = pygame.font.Font(fontName, 82)
        ...
        self.bangSound = pygame.mixer.Sound("Sounds/bangSmall.wav")
        self.fireSound = pygame.mixer.Sound("Sounds/fire.wav")
        self.thrustSoundTime = 60
        self.thrustSoundTimer = 0
        self.thrustSound = pygame.mixer.Sound("Sounds/thrust.wav")
    ...
    def handleInput(self, screen):
        ...
        if not self.gameOver:
            ...
            self.thrustSoundTimer += 1
            if keys[pygame.K_UP]:
                self.player.move()
                if self.thrustSoundTimer > self.thrustSoundTime:
                    self.thrustSoundTimer = 0
                    pygame.mixer.Sound.play(self.thrustSound)

            self.shootTimer += 1
            if keys[pygame.K_SPACE] and self.shootTimer > self.shootTime:
                self.shootTimer = 0
                self.shoot()
                pygame.mixer.Sound.play(self.fireSound)
    ...
    def update(self, screen):
        ...
        if self.gameOver:
            ...
            #Check if there is asteroids left
            ...
            for asteroid in self.asteroids:
                for subAsteroid in asteroid.subAsteroidsVertexIndeces:
                    subAsteroidVertices = []
                    for subVertexIndex in subAsteroid:
                        subAsteroidVertices.append(asteroid.transformedVertices[subVertexIndex])

                    if convexPolygonCollisionSAT(subAsteroidVertices, self.player.transformedVertices):
                        self.explosions.append(Explosion(self.player.position.x, self.player.position.y, 18, 120))
                        pygame.mixer.Sound.play(self.bangLargeSound)
                        self.gameOver = True
            ...
            for asteroid in self.asteroids:
               for bullet in self.bullets:
                   if circleCollision(asteroid.position.x, asteroid.position.y, asteroid.radius, bullet.position.x, bullet.position.y, bullet.radius):
                       ...
                       self.bullets.remove(bullet)
                       pygame.mixer.Sound.play(self.bangSound)
                       #Increse score
                       self.score += 10
                       self.renderScoreToSurface()
                       break
